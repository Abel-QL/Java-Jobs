...
public class SnacksMachine {
    ...
    public static void snackMachine() {
        ...
        IServiceSnacks serviceSnacks = new FileServicesSnack();

        //lista de productos nigga
        List<Snack> snacks = new ArrayList<>();
        System.out.println("--- PUNTO DE DROGA DYLON BABY 69 ---");
        serviceSnacks.showSnack();
        while (!exit) {
            try {
                var option = showMenu(console);
                exit = execOptions(option, console, snacks, serviceSnacks);


            } catch (Exception e) {
                System.out.println("EXPLOTO: " + e.getMessage() + " Tu code no sirve hermano, ¿que le vamos a hacer?");
            } finally {
                System.out.println();
            }
        }
    ...
    private static boolean execOptions(int option, Scanner console,
                                       List<Snack> snacks, IServiceSnacks serviceSnacks) {
        var exit = false;
        switch (option) {
            case 1 -> buySnack(console, snacks, serviceSnacks);
            case 2 -> showSnacks(snacks);
            case 3 -> addSnacks(console, serviceSnacks);
            case 4 -> exit = true;
        }
        return exit;
    }
    ...
    private static void buySnack(Scanner console, List<Snack> snacks, IServiceSnacks serviceSnacks) {
        ...
        if (!snackFound) {
            System.out.println("No se encontró esa vaina manito " + snackId);
        }
    }
    ...
    private static void addSnacks(Scanner console, IServiceSnacks serviceSnacks) {
        System.out.print("Nombre del Snack: ");
        var snackName = console.nextLine();
        System.out.print("Precio: ");
        var snackPrice = Double.parseDouble(console.nextLine());
        serviceSnacks.addSnack(new Snack(snackName, snackPrice));
